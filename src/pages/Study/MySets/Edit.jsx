...
import { Trash2, PlusCircle, Cpu } from 'react-feather'
import { v4 as uuidv4 } from 'uuid'
import useForm from '../../../hooks/useForm'
import { updateSet } from '../../../api/user'
...
function Edit() {
  ...
  const UpdateSet = () => {
    const UpdateSetPromise = new Promise(async (resolve, reject) => {
      const updateResult = await updateSet(localAuth, setId, values)
  ...
  useEffect(() => {
    if (dataIndex >= 0) setValues(userData.userSets[dataIndex])
  }, [dataIndex])

  if (!userData?.userSets)
    return (
      <SpinnerWrapper>
        <Spinner height="3em" />
      </SpinnerWrapper>
